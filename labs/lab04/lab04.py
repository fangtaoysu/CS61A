...
def merge(lst1, lst2):
    ...
    """
    "*** YOUR CODE HERE ***"
    def my_merge(lst1, lst2, res, i, j):
        if len(lst1) <= i and len(lst2) <= j:
            return res
        elif len(lst1) <= i:
            res.append(lst2[j])
            return my_merge(lst1, lst2, res, i, j+1)
        elif len(lst2) <= j:
            res.append(lst1[i])
            return my_merge(lst1, lst2, res, i+1, j)
        if lst1[i] < lst2[j]:
            res.append(lst1[i])
            i += 1
        else:
            res.append(lst2[j])
            j += 1
        return my_merge(lst1, lst2, res, i, j)
    return my_merge(lst1, lst2, [], 0, 0)
...
def count_palindromes(L):
    ...
    2
    """
    return sum(s.lower() == s.lower()[::-1] for s in L)