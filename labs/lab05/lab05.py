...
def replace_loki_at_leaf(t, lokis_replacement):
    ...
    """
    "*** YOUR CODE HERE ***"
    value, branch_list = label(t), []
    if is_leaf(t) and label(t) == 'loki':
        value = lokis_replacement
    for child in branches(t):
        branch_list.append(replace_loki_at_leaf(child, lokis_replacement))
    return tree(value, branch_list)
...
def closer_city(lat, lon, city_a, city_b):
    ...
    return name_a if distance(this_city, city_a) < distance(this_city, city_b) \
                    else name_b
