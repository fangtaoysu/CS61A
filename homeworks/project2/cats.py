...
def autocorrect(typed_word, word_list, diff_function, limit):
    ...
    for word in word_list:
        if word == typed_word:
            return typed_word
        # diff是由diff_function确定的，不需要逐个单词比较
...
def time_per_word(words, timestamps_per_player):
    ...
    # BEGIN PROBLEM 9
    "*** YOUR CODE HERE ***"
    times = [[0] * (len(timestamps_per_player[0])-1) for _ in range(len(timestamps_per_player))]
...
def fastest_words(match):
    ...
    for i in word_indices:
        fast_index = 0
        fast = time(match, fast_index, i)
        for j in range(len(get_all_times(match))-1):
            if fast > time(match, j+1, i):
                fast_index = j + 1
                fast = time(match, j+1, i)

        fastest[fast_index].append(get_word(match, i))
    return fastest
    # END PROBLEM 10
