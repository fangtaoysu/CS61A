...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        while current_place:
            if not current_place.is_hive and len(current_place.bees) > 0:
...
class HungryAnt(Ant):
    name = 'Hungry'
    food_cost = 4
    chewing_turns = 3 # 类属性：每个实例的值都一样
    implemented = True

    def __init__(self, turns_to_chew=0, health=1):
        self.turns_to_chew = turns_to_chew # 实例属性：实例的值可以不一样
        super().__init__(health)

    def action(self, gamestate):
        if self.turns_to_chew > 0:
            self.turns_to_chew += 1
        # 吃到蜜蜂的那一轮turns_to_chew已经为1了，意味着实际值和理论值相差1
        # 如果已经消化了三轮，那么可以继续吃
        # if self.turns_to_chew > (self.chewing_turns + 1):
        #     self.turns_to_chew = 0
        self.turns_to_chew %= self.chewing_turns + 2
...
# END Problem 12
    ...
    def action(self, gamestate):
        ...
        while exit_place:
            if exit_place.ant is not None:
                exit_place.ant.double()
                if exit_place.ant.is_container and exit_place.ant.ant_contained is not None:
                    exit_place.ant.ant_contained.double()
        ...
        while current_place:
            ...
            if current_place.ant is not None and current_place.ant is not self:
                ...
                if current_place.ant.is_container and current_place.ant.ant_contained is not None:
                    res[current_place.ant.ant_containted] = distance
            current_place = current_place.entrance
            distance += 1
        return res
        # END Problem Optional 2
