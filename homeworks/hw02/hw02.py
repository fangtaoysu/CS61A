...
def summation_using_accumulate(n, term):
    ...
    ['Expr', 'Return']
    """
    return reduce(add, (term(i+1) for i in range(n)))
...
def product_using_accumulate(n, term):
    ...
    ['Expr', 'Return']
    """
    return reduce(mul, (term(i+1) for i in range(n)))
...
def funception(func1, begin):
    ...
    def funcep(end):
        beg = begin
        try:
            res = reduce(mul, (func1(i) for i in range(beg, end)))
        except TypeError:
            return 1
        return res
    return funcep
